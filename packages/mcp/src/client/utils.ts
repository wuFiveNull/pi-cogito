/** Minimal host surface the utilities need (subset of the pi ExtensionAPI). */
...
async function execOpen(pi: ExecHost, target: string, browser?: string, signal?: AbortSignal) {
	const os = platform();
...
export async function openUrl(pi: ExecHost, url: string, browser?: string, signal?: AbortSignal): Promise<void> {
...
export async function openPath(pi: ExecHost, targetPath: string): Promise<void> {
...
export function formatAuthRequiredMessage(
	...
	// biome-ignore lint/suspicious/noTemplateCurlyInString: 保留 ${server} 占位符供 replaceAll
