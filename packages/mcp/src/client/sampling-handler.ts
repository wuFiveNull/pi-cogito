import type { Api, AssistantMessage, Message, Model, TextContent } from "@cogito/ai";

/**
 * Sampling backend. mcp-client-core does not bundle a model runtime; hosts
 * must install a completeFn (e.g. via pi-ai) when MCP sampling is needed.
 * Without one, sampling requests are rejected.
 */
export type SamplingCompleteFn = (params: {
	model: Model<Api>;
	apiKey?: string;
...
/** Minimal model registry surface (subset of the pi ModelRegistry). */
...
import type { ExtensionUIContext } from "./runtime-owner.ts";
...
export interface SamplingHandlerOptions {
	...
	/** Sampling backend. When absent, sampling requests are rejected. */
	completeFn?: SamplingCompleteFn;
...
export async function handleSamplingRequest(
	...
	if (!options.completeFn) {
		throw new Error("MCP sampling is not available: no sampling backend configured");
	}
	const result = await options.completeFn({
		model,
		apiKey,
		headers: headers ?? {},
