...
- In-query compaction is now persisted across turns (akashic `react_compaction` metadata equivalent): each compaction emits a hidden custom `query_compaction` message (display=false) carrying `{startIndex, coveredCount, contextWindow}` that the host persists into the session; `runAgentLoop`/`runAgentLoopContinue` replay the markers at turn entry — covered messages are dropped and the summary is projected at its original coordinates (A_i = startIndex_i - i + sum of prior covered counts). Bounds-checked fail-safe: when host turn-level compaction already removed the covered range, the original messages are kept and nothing is dropped.
- Added in-query compaction (`query-compaction.ts`, akashic `query_compaction.py` QueryCompactor port): during a long single-request tool loop, when the estimated context exceeds `contextWindow * triggerPercent` (default 0.74), earlier completed batches are replaced by a structured summary projected as a marked user message (`[上下文压缩]`), keeping recent active batches; wired via `AgentLoopConfig.queryCompaction` in `runLoop`, with pluggable `summarize`/`estimate` and a default main-model summarizer.
- Added recent-context compression (`recent-context.ts`, akashic `_render_recent_context` port): consolidation now refreshes `RECENT_CONTEXT.md` with five-field LLM compression (active topics / explicit preferences / follow-ups / avoidances / ongoing threads) plus Recent Turns previews; enabled on the periodic `ConsolidationLoop` path (`writeRecentContext`), kept off the per-turn `before_turn` path to avoid extra LLM calls per turn.
...
- Moved the `uuidv7` export to `@cogito/ai` ([#6834](https://github.com/earendil-works/pi/pull/6834) by [@xl0](https://github.com/xl0)).
- Replaced the optional `Agent` `streamFn` fallback with a required `streamFunction` and made low-level loop stream functions required, preventing `@cogito/ai/compat` and all built-in providers from entering selective-provider bundles ([#6851](https://github.com/earendil-works/pi/issues/6851)).
...
- Removed the `@cogito/agent-core/base` selective-provider entrypoint; use the root package with an explicit `Models` instance instead.
...
- Added `@cogito/agent-core/base` for bundlers that want to pair the agent core with selective `@cogito/ai/base` provider registration ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
