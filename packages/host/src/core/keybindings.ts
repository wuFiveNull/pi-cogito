...
/** A keybinding: either a legacy name string or a full definition. */
export type Keybinding = string | KeybindingDefinition;

export interface KeybindingDefinitions {
	[key: string]: KeybindingDefinition;
}

export type KeyId = string;

export type KeybindingsConfig = Record<string, string | string[]>;
...
/**
 * Headless keybindings manager. pi-host has no TUI, so this class only stores
 * the definitions and the user-provided bindings and resolves them on demand;
 * nothing consumes the resolved bindings at runtime.
 */
export class KeybindingsManager {
	...
	getResolvedBindings(): KeybindingsConfig {
		...
		for (const [key, definition] of Object.entries(KEYBINDINGS)) {
			if (Object.hasOwn(this.userBindings, key)) {
				resolved[key] = this.userBindings[key];
			} else if (definition.defaultKeys.length > 0) {
