...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".cogito", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".cogito", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".cogito", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".cogito"), { recursive: true });
			writeFileSync(join(cwd, ".cogito", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".cogito"), { recursive: true, force: true });
