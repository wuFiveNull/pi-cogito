...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@cogito/host")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @cogito/host",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@cogito/host")).toBeUndefined();
		expect(getUpdateInstruction("@cogito/host")).toBe(
			"Update @cogito/host using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@cogito/host");

		expect(detectInstallMethod()).toBe("npm");
		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@cogito/host"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @cogito/host`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@cogito/host", undefined, {
			packageName: "@cogito/host",
			installSpec: "@cogito/host@1.2.3",
		});

		expect(command).toEqual({
			command: "npm",
			args: ["--prefix", prefix, "install", "-g", "--ignore-scripts", "--min-release-age=0", "@cogito/host@1.2.3"],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @cogito/host@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@cogito/host", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @cogito/host`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@cogito/host")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @cogito/host",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@cogito/host"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @cogito/host",
		});
	});
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@cogito/host")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@cogito/host")).toContain("the install path is not writable");
	});
});
