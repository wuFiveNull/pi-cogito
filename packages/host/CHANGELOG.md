...
- Added the AI approval judge (`core/approval/judge.ts`, exported from `@cogito/host` as `ApprovalJudge`/`createLlmApprovalJudge`/`parseVerdict`): one-shot LLM verdicts (`allow`/`deny` + minimal rule + reason) for blocked actions, with per-(kind, target) verdict caching, a per-session call budget, per-verdict timeout, and JSON parsing that fails closed — malformed output, timeouts, an exhausted budget, or a missing/unauthenticated model all deny. Model resolution: `approval.model` ("provider/model-id") or the first catalog model with configured auth, via a lazily shared `ModelRuntime`.
- Added the OS-level sandbox extension (ported from [pi-sandbox](https://github.com/carderne/pi-sandbox), MIT, based on badlogic/pi-mono's sandbox example): bash commands are wrapped with `@carderne/sandbox-runtime` (sandbox-exec on macOS, bubblewrap on Linux) to enforce network domain allowlists and filesystem restrictions at the OS level; read/write/edit tool calls are intercepted against the same filesystem policy. Blocked actions are reviewed solely by the AI approval judge — there is no human review path; allow verdicts grant session-level rules only (config files are never written by verdicts) and deny/failure blocks feed the AI reason back to the agent. Each resolution is recorded as a `sandbox-approval` audit entry. Implementation lives in `core/sandbox/` (`policy`/`config`/`runtime`/`approval`/`sandbox-extension`), packaged as `createSandboxExtension()` (`InlineExtension`, exported from `@cogito/host`) and mounted on the default resource loader in `createAgentSession`; consumers with a custom loader add it to `extensionFactories`. Inert unless `sandbox.json` sets `enabled: true`. Requires `socat` on Linux.
...
- Added `ExtensionSqlite` to the public exports (needed by `@cogito/chat` for extension audit logging).
- Added a host-layer tool catalog with keyword search (`ToolCatalog`/`KeywordSearchBackend` in `core/tool-catalog.ts`, exported from `@cogito/host`): scores tool entries by name parts (exact/partial/fallback 10/5/3), `searchHint` (+4) and description (+2), with CJK bigram + char-level substring handling and per-hit `whyMatched` reasons. The catalog is maintained incrementally at the registration collection point: builtin tools are fully indexed at startup, and `registerTool`/reload diffs add/remove entries without a restart.
- Added the optional `searchHint` field to `ToolDefinition` (space-separated trigger words such as `"文件 阅读 查看 file read"`); builtin and chat tools ship example hints.
- Added the `tool_search` tool, mounted as a hidden inline extension in every `AgentSession`: searches the catalog (`query` required, `limit` default 8) and activates matched tools via `setActiveTools`, so their definitions are exposed on the next request (native deferred loading on capable models, regular tool list otherwise).
- Added host-layer subagent delegation: `SubagentManager` (background `spawn`/sync `spawnSync`, concurrency cap default 3 via `SubagentCapacityError`, per-job timeout, cancel, shutdown), `createSubagentAgentRunner` (isolated in-process sub-agent with read-only default tools and iteration cap), `createSpawnTool`/`createSpawnManageTool` tool definitions, and `createSubagentExtension` to mount them via `registerTool` (background completions delivered back to the origin session as a new turn).
- Added the consolidation bridge (`ConsolidationBridge` in `core/memory/consolidation-bridge.ts`, exported from `@cogito/host`): syncs markdown-layer consolidation output into the memory engine — `history_entries` as `event` items via `saveFromConsolidation`/`upsertConsolidationEvent` (source_ref idempotent + semantic dedup) and a second-pass LLM implicit long-term extraction (profile/preference/procedure via `saveItemWithSupersede`, USER-original-words only, akashic `_extract_implicit_long_term` port), preserving the source session `channel`/`chatId` scope; skips vector writes entirely when no embedder is configured.
- Added batch verdicts to the AI approval judge (`judgeBatch` on `ApprovalJudge`, native in `createLlmApprovalJudge`, plus `resolveApprovalBatch` in `core/sandbox/approval.ts`): several targets reviewed with a single model call and one budget unit; parses per-target verdict arrays (including `{"verdicts":[...]}` and single-object replies), caches per (kind, target), and treats unanswered targets as denied. Sandbox bash preflight now judges all non-whitelisted URL domains of a command in one batch, applies grants with a single sandbox reinit, and blocks with per-target reasons; judges without `judgeBatch` fall back to per-target calls.
- Added connection-level network review to the sandbox extension: the sandbox runtime's ask callback now consults the approval judge for any host that misses the network whitelist (config `allowedDomains` plus session-granted rules) at connect time, covering bare domains, raw IPs, and hosts that are not URL-shaped in the command text. Ask-time grants are recorded as session allowances without a mid-command sandbox reinit; a denied or unavailable judge fails the connection closed.
...
- Fixed the process-wide `SandboxManager` being torn down — or rebuilt with only the most recent session's config and grants — while other sessions of the same host process still relied on it: every enabled session now holds a sandbox lease (`acquireSandboxLease`/`releaseSandboxLease` in `core/sandbox/runtime.ts`); the OS sandbox is rebuilt from the union of all active leases' configs and allowances, and is reset only when the last lease is released, so one session exiting no longer silently strips the others' sandboxing.
...
- Removed inherited selective-provider `@cogito/ai/base` and `@cogito/agent-core/base` entrypoints; use the root packages with explicit `Models` provider factories instead.
...
- Added inherited `@cogito/ai/base` and `@cogito/agent-core/base` entry points for selective provider registration in bundled applications ([#5348](https://github.com/earendil-works/pi/pull/5348) by [@FredKSchott](https://github.com/FredKSchott)).
...
- Added Claude Fable 5 model support inherited from `@cogito/ai` for the Anthropic and Amazon Bedrock providers, with adaptive thinking and `xhigh` effort support.
...
- Added MiniMax-M3 model support inherited from `@cogito/ai` for the `minimax` and `minimax-cn` direct providers ([#5313](https://github.com/earendil-works/pi/issues/5313)).
...
- Added custom Amazon Bedrock request header support inherited from `@cogito/ai` ([#5178](https://github.com/earendil-works/pi-mono/pull/5178) by [@stephanmck](https://github.com/stephanmck)).
...
- Fixed Amazon Bedrock provider loading under strict package managers by inheriting the declared `@smithy/node-http-handler` dependency from `@cogito/ai` ([#4842](https://github.com/earendil-works/pi/issues/4842)).
...
- Fixed loader indicator startup inherited from `@cogito/tui` so initialization cannot run before frames are available.
- Fixed OpenAI-compatible default output token requests inherited from `@cogito/ai` to avoid reserving impossible context windows on servers such as vLLM ([#4675](https://github.com/earendil-works/pi/issues/4675)).
- Fixed OpenAI prompt cache keys inherited from `@cogito/ai` to stay within the 64-character provider limit ([#4720](https://github.com/earendil-works/pi/issues/4720)).
...
- Fixed Xiaomi MiMo generated model metadata to replay assistant tool-call messages with `reasoning_content` for thinking-mode multi-turn requests, inherited from `@cogito/ai` ([#4678](https://github.com/earendil-works/pi/issues/4678)).
...
- Fixed Anthropic-compatible API-key requests to ignore unrelated `ANTHROPIC_AUTH_TOKEN` environment values, avoiding invalid bearer credentials for providers such as Xiaomi MiMo inherited from `@cogito/ai` ([#4342](https://github.com/earendil-works/pi/issues/4342)).
- Fixed Amazon Bedrock message conversion to skip unknown content blocks instead of failing the stream, inherited from `@cogito/ai` ([#4223](https://github.com/earendil-works/pi/issues/4223)).
- Fixed Azure OpenAI Responses and OpenAI Responses error formatting to prefix HTTP status codes onto `errorMessage`, so transient 5xx and 429 errors are correctly matched by the agent-level auto-retry classifier inherited from `@cogito/ai` ([#4232](https://github.com/earendil-works/pi/issues/4232)).
- Fixed OpenCode Go Kimi reasoning replay by normalizing streamed `reasoning` fields back to `reasoning_content` for OpenCode Go only, inherited from `@cogito/ai` ([#4251](https://github.com/earendil-works/pi/issues/4251)).
- Fixed Xiaomi MiMo model metadata to use the OpenAI-compatible endpoints and `openai-completions` API, restoring multi-turn thinking/tool-call sessions inherited from `@cogito/ai` ([#4505](https://github.com/earendil-works/pi/issues/4505)).
...
- Removed non-working OpenAI Codex fast model variants inherited from `@cogito/ai`.
...
- Fixed OpenAI Codex generated model metadata to use the current upstream model list inherited from `@cogito/ai` ([#4603](https://github.com/earendil-works/pi-mono/pull/4603) by [@mattiacerutti](https://github.com/mattiacerutti)).
- Fixed GitHub Copilot GPT model thinking metadata inherited from `@cogito/ai` to map unsupported minimal thinking to low ([#4622](https://github.com/earendil-works/pi-mono/pull/4622) by [@mattiacerutti](https://github.com/mattiacerutti)).
...
- Fixed default output token requests for models whose advertised output limit is effectively their full context window, avoiding impossible provider requests inherited from `@cogito/ai` ([#4614](https://github.com/earendil-works/pi/issues/4614)).

## [0.74.1] - 2026-05-16

### New Features

- **Image generation support** - Added image generation APIs, generated image model metadata, and built-in OpenRouter image generation support inherited from `@cogito/ai`.
...
- **Improved terminal and markdown rendering** - Added markdown list indentation, task-list checkbox rendering, large markdown robustness, and inline image placement fixes inherited from `@cogito/tui`.

### Added

- Added image generation support from `@cogito/ai`, including image generation APIs, image model metadata, and built-in OpenRouter image generation support ([#3887](https://github.com/earendil-works/pi-mono/pull/3887) by [@cristinaponcela](https://github.com/cristinaponcela)).
...
- Fixed OpenRouter cached-token usage accounting, Fireworks caching compatibility, and OpenAI Codex WebSocket proxy handling inherited from `@cogito/ai`.
- Fixed markdown list wrapping, task-list checkboxes, large markdown rendering, WezTerm Kitty keyboard escape handling, and short-viewport inline image placement inherited from `@cogito/tui`.
