...
```bash
npm install @cogito/ai
```

TypeBox exports are re-exported from `@cogito/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@cogito/ai';
...
import { openrouterProvider } from '@cogito/ai/providers/openrouter';
import { amazonBedrockProvider } from '@cogito/ai/providers/amazon-bedrock';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@cogito/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@cogito/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@cogito/ai';
...
```typescript
import { validateToolCall, type Tool } from '@cogito/ai';
...
```typescript
import { builtinImagesModels } from '@cogito/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@cogito/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@cogito/ai';
import { openAICompletionsApi } from '@cogito/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@cogito/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@cogito/ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@cogito/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@cogito/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@cogito/ai';
import { anthropicProvider } from '@cogito/ai/providers/anthropic';
import { openaiProvider } from '@cogito/ai/providers/openai';
import { googleProvider } from '@cogito/ai/providers/google';
...
- `@cogito/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@cogito/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@cogito/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@cogito/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@cogito/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@cogito/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@cogito/ai/bedrock-provider';
...
```bash
npx @cogito/ai login              # interactive provider selection
npx @cogito/ai login anthropic    # login to specific provider
npx @cogito/ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@cogito/ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@cogito/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@cogito/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@cogito/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@cogito/ai`.
